...
if __name__ == '__main__':
    seed_everything(42)
    device='cpu'
    # in this course, auto has been alter to max or min? what monitor mean in the val_accuracy()
    early_stop_callback=EarlyStopping(monitor='val_accuracy',min_delta=0.01,patience=5,verbose=True,mode='max')
    model=OurModel().to(device)
    trainer=Trainer(max_epochs=100,min_epochs=10,auto_lr_find=True, auto_scale_batch_size=True,
